...
// REMOVED: import { TICKETMASTER_API_KEY, GOOGLE_PLACES_API_KEY } from '../config';

// Access API keys from environment variables
// Keys are defined in the .env file for local development
// Prefix with EXPO_PUBLIC_ to make them available in client-side code bundle
// Fallback logic removed assuming .env setup or EAS secrets injection
...
export default function IndexScreen() {
    ...
    const handleCityChange = async (text: string) => {
        ...
        if (!googleApiKey) { console.warn("Google API Key missing or not exposed. Check .env and prefix (EXPO_PUBLIC_)"); return; } // Use const defined above
        if (text.length > 2) {
    ...
    const handleConcertSearch = async () => {
        ...
        // Use consts defined above for key checks
        if (!selectedPlaceId || !startDate || !endDate) { Alert.alert("Validation Error", "Please select city and both dates."); return; }
        if (!ticketmasterApiKey || !googleApiKey) { Alert.alert("API Key Error", "API keys not loaded. Check .env file and prefixes (EXPO_PUBLIC_)"); return; }
        ...
        try {
            ...
            /* ... fetch ... */
            console.log("Fetching Geocoding URL:", geocodingUrl); const geoResponse = await fetch(geocodingUrl); const geoData = await geoResponse.json(); if (geoData.status !== 'OK' || !geoData.results || geoData.results.length === 0) { throw new Error(`Geocoding failed: ${geoData.status} ...`); } const location = geoData.results[0].geometry.location; const lat = location.lat; const lng = location.lng; console.log(`Coordinates for ${city}:`, lat, lng);
            ...
            if (tmData._embedded && tmData._embedded.events) {
                ...
                // console.log("Fetched Event Local Dates:", fetchedEvents.map(e => e.dates?.start?.localDate)); // Keep commented unless debugging date filter
            } else {
            ...
            console.log(`Filtering results between ${userStartDateString} and ${userEndDateString}...`);
            const filteredEvents = fetchedEvents.filter(event => {
                const eventLocalDate = event.dates?.start?.localDate;
                const keep = eventLocalDate && eventLocalDate >= userStartDateString && eventLocalDate <= userEndDateString;
                // console.log(`Filtering Event: ${event.name}, Date: ${eventLocalDate}, Keep: ${keep}`); // Keep commented unless debugging
                return keep;
            });
