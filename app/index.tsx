...
// Keys are defined in the .env file for local development
// EAS Build automatically picks up .env OR uses secrets set via `eas secret:create`
...
export default function IndexScreen() {
    ...
    const handleCityChange = async (text: string) => {
        setCity(text); setSelectedPlaceId(null); setSuggestions([]); setShowSuggestions(false);
        // Use variable defined above
        if (!googleApiKey) { console.warn("Google API Key missing..."); return; }
        if (text.length > 2) {
            setIsCityLoading(true);
            const googleApiUrl = `https://maps.googleapis.com/maps/api/place/autocomplete/json?input=${encodeURIComponent(text)}&types=(cities)&components=country:us&key=${googleApiKey}`;
            try { /* ... fetch logic ... */ const response = await fetch(googleApiUrl); const data = await response.json(); if (data.status === 'OK') { setSuggestions(data.predictions); setShowSuggestions(true); } else { setSuggestions([]); setShowSuggestions(false); if (data.status !== 'ZERO_RESULTS') console.error("Google API Error:", data.status); } } catch (error) { console.error("Google fetch Error:", error); setSuggestions([]); setShowSuggestions(false); } finally { setIsCityLoading(false); }
    ...
    const handleConcertSearch = async () => {
        ...
        // Use variables defined above for key checks
        if (!selectedPlaceId) { Alert.alert("Missing Location", "..."); return; }
        if (!startDate || !endDate) { Alert.alert("Missing Dates", "..."); return; }
        if (!ticketmasterApiKey || !googleApiKey) { Alert.alert("Missing API Key(s)", "Please ensure API keys are set."); return; }

        setIsConcertLoading(true); setConcertError(null); setConcerts([]);
        try {
            // Step A: Geocoding Call using googleApiKey
            const geocodingUrl = `https://maps.googleapis.com/maps/api/geocode/json?place_id=${selectedPlaceId}&key=${googleApiKey}`;
            /* ... fetch geoResponse ... */
            const geoResponse = await fetch(geocodingUrl); const geoData = await geoResponse.json(); if (geoData.status !== 'OK' || !geoData.results || geoData.results.length === 0) { throw new Error(`Geocoding failed: ${geoData.status} ...`); } const location = geoData.results[0].geometry.location; const lat = location.lat; const lng = location.lng;

            // Step B: Ticketmaster Call using ticketmasterApiKey
            const radius = 30; const unit = "miles";
            const startDateTime = startDate.toISOString().slice(0, 10) + 'T00:00:00Z';
            const endDateTime = endDate.toISOString().slice(0, 10) + 'T23:59:59Z';
            const ticketmasterApiUrl = `https://app.ticketmaster.com/discovery/v2/events.json?apikey=${ticketmasterApiKey}&latlong=${lat},${lng}&radius=${radius}&unit=${unit}&startDateTime=${startDateTime}&endDateTime=${endDateTime}&sort=date,asc&classificationName=Music&size=100`;
            /* ... fetch tmResponse ... */
             const tmResponse = await fetch(ticketmasterApiUrl); if (!tmResponse.ok) { let errorMsg = `TM HTTP error! Status: ${tmResponse.status}`; try { const errorData = await tmResponse.json(); errorMsg += `: ${errorData?.fault?.faultstring || errorData?.errors?.[0]?.detail || 'Unknown TM error'}`; } catch (e) {} throw new Error(errorMsg); } const tmData = await tmResponse.json(); if (tmData._embedded && tmData._embedded.events) { setConcerts(tmData._embedded.events); } else { setConcerts([]); }

        } catch (err: any) { console.error("API Fetch Error:", err); setConcertError(`Search failed: ${err.message}`);
        } finally { setIsConcertLoading(false); }
    };
    // ---

    // --- UI Layout ---
    // No changes needed in the JSX structure itself
    return (
